function upsertMediaItems(db, files, runId) {
  ...
  const upsert = db.prepare(`
    INSERT INTO media_items (path, ext, media_type, size, mtime_ms, resolved_time_ms, resolved_source, latitude, longitude, location_source, place_name, ai_tags, face_count, embedding, thumbnail_path, faces_indexed, visual_indexed, person_class, confidence, last_seen_run, is_missing)
    VALUES (@path, @ext, @mediaType, @size, @mtimeMs, @resolvedTimeMs, @resolvedSource, @latitude, @longitude, @locationSource, @placeName, @aiTags, @faceCount, @embedding, @thumbnailPath, @facesIndexed, @visualIndexed, @personClass, @confidence, @lastSeenRun, 0)
...
function replaceEvents(db, events) {
  const existingEvents = db.prepare(
    'SELECT id, start_time_ms, end_time_ms, item_count FROM events'
  ).all();
...
function classifyPersonPresence(analysis) {
  ...
  if (!Array.isArray(detections)) return 'none';
  const personDetections = detections.filter(d => d.label === 'person' && d.score >= 0.5);
  if (personDetections.length === 0) return 'none';
  if (personDetections.length === 1) return 'portrait';
  return 'group';
}
...
function updateMediaVisualAnalysis(db, mediaId, analysis, options = {}) {
  ...
  const personClass = classifyPersonPresence(analysis);
  return db.prepare(`
    UPDATE media_items
    SET ai_tags = ?,
        face_count = ?,
